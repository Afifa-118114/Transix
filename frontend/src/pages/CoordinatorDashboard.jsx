...
export default function CoordinatorDashboard() {
  ...
  // Theme-aware UI matching Transix design system
  return (
    <DashboardLayout trip={trip} setTrip={setTrip}>
      <div className="w-full text-slate-800 dark:text-slate-200 pb-12 font-sans transition-colors duration-200">
        <div className="max-w-[1400px] mx-auto px-4 md:px-8 pt-6">
          
          {/* Breadcrumb & Trip Type */}
          <div className="flex items-center gap-3 mb-4">
            <div className="flex items-center gap-2 px-3 py-1 bg-indigo-50 dark:bg-indigo-600/20 text-indigo-600 dark:text-indigo-400 rounded-full text-xs font-bold border border-indigo-200 dark:border-indigo-500/30">
              <span className="w-2 h-2 rounded-full bg-indigo-500 animate-pulse"></span>
              Campus IV
            </div>
            <div className="px-3 py-1 bg-slate-100 dark:bg-slate-800/80 text-slate-700 dark:text-slate-300 rounded-full text-xs font-semibold border border-slate-200 dark:border-slate-700">
              Educational Trip
            </div>
          </div>

          {/* Hero Section */}
          <div className="relative rounded-2xl overflow-hidden mb-4 border border-slate-200 dark:border-slate-800 bg-gradient-to-r from-indigo-50/90 via-white to-slate-50 dark:from-[#0a101f] dark:via-[#0f172a] dark:to-transparent shadow-xs dark:shadow-2xl">
            {/* Background image effect */}
            <img 
              src="https://images.unsplash.com/photo-1593693397690-362cb9666cb2?auto=format&fit=crop&q=80&w=2000" 
              alt="Destination" 
              className="absolute inset-0 w-full h-full object-cover opacity-10 dark:opacity-20 mix-blend-overlay z-0"
            />
            ...
            <div className="relative z-10 p-6 md:p-8 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
              <div>
                ...
                <div className="flex flex-wrap items-center gap-3 text-xs font-semibold text-slate-600 dark:text-slate-300">
                  <div className="flex items-center gap-1.5 bg-white/80 dark:bg-slate-800/60 px-2.5 py-1 rounded-md border border-slate-200 dark:border-slate-700/50 shadow-xs">
              ...
              {trip.status === "Finalized" && (
                <div className="shrink-0 bg-white/95 dark:bg-[#131c31] border border-slate-200 dark:border-slate-700 p-4 rounded-xl shadow-md backdrop-blur-md">
                  {trip.joinCode && <p className="text-[10px] text-slate-500 dark:text-slate-400 font-bold uppercase mb-1">IV Code</p>}
                  <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                    {trip.joinCode && (
                      <h2 className="text-2xl font-black text-slate-900 dark:text-white tracking-widest sm:mr-2">{trip.joinCode}</h2>
                    )}
                    <div className="flex flex-wrap items-center gap-2">
                      {trip.joinCode && (
                        <button 
                          onClick={handleCopyIvCode} 
                          className="flex items-center gap-1.5 p-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 rounded-lg border border-slate-200 dark:border-slate-700 transition cursor-pointer text-slate-700 dark:text-slate-300" 
                          title={copiedIvCode ? "Copied!" : "Copy IV Code"}
                        >
                          {copiedIvCode ? (
                            <>
                              <FiCheck className="text-indigo-600 dark:text-indigo-400" />
                              <span className="text-[10px] font-bold text-indigo-600 dark:text-indigo-400">Copied!</span>
                            </>
                          ) : (
                            <FiCopy className="text-slate-500 dark:text-slate-300" />
                          )}
                        </button>
                      )}
                      <button 
                        ...
                        className="flex items-center gap-2 px-3 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold rounded-lg transition shadow-md shadow-indigo-600/30 cursor-pointer disabled:opacity-60"
                      ...
                      <button
                        ...
                        className="relative flex items-center gap-2 px-3 py-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-750 text-slate-800 dark:text-white border border-slate-200 dark:border-slate-700 hover:border-indigo-500 text-xs font-bold rounded-lg transition shadow-xs group"
                        title="Direct chat with Transix Tour Operations"
                      >
                        <FiMessageSquare className="text-indigo-600 dark:text-indigo-400 group-hover:text-indigo-500" size={14} />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 md:gap-6">
            ...
            <div className="lg:col-span-3 flex flex-col gap-4 md:gap-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                ...
                <div className="bg-white dark:bg-[#131c31] border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-xs dark:shadow-lg flex flex-col transition-colors">
                  <div className="flex justify-between items-center mb-6">
                    <h3 className="text-sm font-bold text-slate-900 dark:text-white flex items-center gap-2">
                      <FiPieChart className="text-indigo-600 dark:text-indigo-400" />
                      Registration Overview
                    </h3>
                  </div>
                  
                  <div className="flex items-center justify-between flex-1">
                    {/* Donut Chart via CSS / SVG */}
                    <div className="relative w-32 h-32 shrink-0">
                      <svg className="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
                        <circle cx="50" cy="50" r="40" fill="none" className="stroke-slate-200 dark:stroke-slate-800" strokeWidth="12" />
                        <circle cx="50" cy="50" r="40" fill="none" stroke="#6366f1" strokeWidth="12" strokeDasharray={`${Math.min((registeredCount / (settings.capacity || 1)) * 251, 251)} 251`} strokeLinecap="round" />
                      </svg>
                      <div className="absolute inset-0 flex flex-col items-center justify-center">
                        <span className="text-xl font-black text-slate-900 dark:text-white">{registeredCount} <span className="text-sm text-slate-400 dark:text-slate-500">/ {settings.capacity || '-'}</span></span>
                        <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase">Registered</span>
                      </div>
                    </div>
                    ...
                    <div className="flex-1 ml-6 space-y-3 text-xs font-semibold">
                      <div className="flex justify-between items-center">
                        <span className="flex items-center gap-2 text-slate-700 dark:text-slate-300"><span className="w-2.5 h-2.5 rounded-full bg-indigo-600"></span> Registered</span>
                        <span className="text-slate-900 dark:text-white font-bold">{registeredCount}</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="flex items-center gap-2 text-slate-700 dark:text-slate-300"><span className="w-2.5 h-2.5 rounded-full bg-sky-500"></span> Pending</span>
                        <span className="text-slate-900 dark:text-white font-bold">{pendingCount}</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="flex items-center gap-2 text-slate-700 dark:text-slate-300"><span className="w-2.5 h-2.5 rounded-full bg-amber-500"></span> Under Review</span>
                        <span className="text-slate-900 dark:text-white font-bold">{underReviewCount}</span>
                      </div>
                      <div className="flex justify-between items-center">
                ...
                <div className="bg-white dark:bg-[#131c31] border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-xs dark:shadow-lg transition-colors">
                  <div className="flex justify-between items-center mb-6">
                    ...
                    <button onClick={() => { setSettingsModalTab("general"); setShowRegistrationModal(true); }} className="text-[10px] font-bold uppercase text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 transition">Edit Settings</button>
                  </div>
                  
                  <div className="grid grid-cols-2 gap-y-6 gap-x-4 mb-6">
                    <div>
                      <p className="text-[10px] font-bold text-slate-500 uppercase mb-1">Registration Opens</p>
                      <p className="text-sm font-semibold text-slate-900 dark:text-white">{settings.openDate ? new Date(settings.openDate).toLocaleDateString('en-GB', {day:'2-digit', month:'short', year:'numeric'}) : 'Not set'}</p>
                    </div>
                    <div>
              ...
              <div className="bg-white dark:bg-[#131c31] border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xs dark:shadow-lg overflow-hidden flex flex-col transition-colors">
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    <div className="relative">
                      ...
                      <input 
                        ...
                        className="bg-slate-50 dark:bg-[#0a101f] border border-slate-300 dark:border-slate-700 text-xs text-slate-900 dark:text-white rounded-lg pl-8 pr-3 py-1.5 focus:outline-none focus:border-indigo-500 placeholder-slate-400 dark:placeholder-slate-500 w-48"
                      />
                    </div>

                    <div className="flex items-center gap-1 bg-slate-100 dark:bg-[#0a101f] border border-slate-200 dark:border-slate-700 rounded-lg p-0.5 text-xs">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left">
                    <thead className="bg-slate-50 dark:bg-[#0a101f] border-b border-slate-200 dark:border-slate-800">
                      <tr>
                        <th className="px-6 py-4 text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase">Student Details</th>
                        <th className="px-6 py-4 text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase">Registration</th>
                        <th className="px-6 py-4 text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase">Documents</th>
                        <th className="px-6 py-4 text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase">Confirmation Fee</th>
                        <th className="px-6 py-4 text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase">Coordinator Review</th>
                        <th className="px-6 py-4 text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase text-right">Action</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-200 dark:divide-slate-800 text-xs">
                      {filteredStudents.length === 0 ? (
                        ...
                        filteredStudents.map(student => {
                          ...
                          return (
                            <tr key={student._id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/30 transition">
                              <td className="px-6 py-4">
                                <div className="flex items-center gap-3">
                                  <div className="w-9 h-9 rounded-full bg-indigo-100 dark:bg-indigo-900/60 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-500/30 flex items-center justify-center font-bold text-xs shrink-0">
                                    {name.substring(0,2).toUpperCase()}
                                  </div>
                                  <div>
                                    <div className="font-bold text-slate-900 dark:text-white text-sm">{name}</div>
                                    <div className="text-[11px] text-slate-500 dark:text-slate-400 flex items-center gap-2 mt-0.5">
                                      {rollNo && <span className="bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded font-mono text-[10px] text-slate-700 dark:text-slate-300">{rollNo}</span>}
                                      {dept && <span>{dept}</span>}
                                      {year && <span>• {year}</span>}
                                    </div>
                              ...
                              <td className="px-6 py-4">
                                <div className={`flex items-center gap-1.5 font-semibold ${allVerified ? 'text-indigo-600 dark:text-indigo-400' : 'text-amber-600 dark:text-amber-400'}`}>
                              ...
                              <td className="px-6 py-4">
                                <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[11px] font-bold ${
                                  isPaid ? 'bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border border-indigo-200 dark:border-indigo-500/20' : 'bg-amber-50 dark:bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-200 dark:border-amber-500/20'
                                }`}>
                              ...
                              <td className="px-6 py-4">
                                <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[11px] font-bold ${
                                  reviewStatus === 'APPROVED' ? 'bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border border-indigo-200 dark:border-indigo-500/20' :
                                  reviewStatus === 'REJECTED' ? 'bg-rose-50 dark:bg-rose-500/10 text-rose-600 dark:text-rose-400 border border-rose-200 dark:border-rose-500/20' :
                              ...
                              <td className="px-6 py-4 text-right">
                                <button 
                                  ...
                                  className="px-3.5 py-1.5 bg-indigo-50 dark:bg-indigo-600/20 hover:bg-indigo-100 dark:hover:bg-indigo-600/30 text-indigo-600 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-500/30 rounded-lg font-bold text-xs transition shadow-xs cursor-pointer"
                                >
                                  Review Student
                                </button>
                              </td>
              ...
              <div className="bg-white dark:bg-[#131c31] border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-xs dark:shadow-lg flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition-colors">
                <div>
                  <h3 className="text-sm font-bold text-slate-900 dark:text-white flex items-center gap-2">
                  ...
                  <div className="flex flex-wrap items-center gap-2 mt-3 text-[11px] font-semibold">
                    <span className="bg-slate-50 dark:bg-[#0a101f] border border-slate-200 dark:border-slate-800 px-2.5 py-1 rounded-md text-slate-700 dark:text-slate-300">
                      {activeFormFields.length} fields configured
                    </span>
                    <span className="bg-indigo-50 dark:bg-[#0a101f] border border-indigo-200 dark:border-slate-800 px-2.5 py-1 rounded-md text-indigo-600 dark:text-indigo-400">
                      {activeFormFields.filter(f => f.required).length} required fields
                    </span>
                    <span className="bg-slate-50 dark:bg-[#0a101f] border border-slate-200 dark:border-slate-800 px-2.5 py-1 rounded-md text-slate-600 dark:text-slate-400">
                ...
                <button
                  ...
                  className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-xs font-bold transition flex items-center gap-2 shadow-md shadow-indigo-600/30 shrink-0 cursor-pointer"
                >
            ...
            <div className="flex flex-col gap-4 md:gap-6">
              ...
              <div className="bg-white dark:bg-[#131c31] border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-xs dark:shadow-lg transition-colors">
                <div className="flex justify-between items-center mb-6">
                  ...
                  <button 
                    ...
                    className="text-[10px] font-bold uppercase text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 transition cursor-pointer"
                  >
                    Edit Budget
                  </button>
                </div>
                ...
                <div className="space-y-4 mb-6">
                  <div className="flex justify-between items-center text-xs">
                    <span className="text-slate-500 dark:text-slate-400 flex items-center gap-2"><FiFileText /> Total Fee <span className="text-[10px] italic">(per student)</span></span>
                    <span className="font-bold text-slate-900 dark:text-white">₹{settings.totalFee?.toLocaleString() || 0}</span>
                  </div>
                  <div className="flex justify-between items-center text-xs">
                    <span className="text-slate-500 dark:text-slate-400 flex items-center gap-2"><FiCheckCircle /> Confirmation Fee</span>
                    <span className="font-bold text-indigo-600 dark:text-indigo-400">₹{settings.confirmationFee?.toLocaleString() || 0}</span>
                  </div>
                  <div className="flex justify-between items-center text-xs pt-4 border-t border-slate-200 dark:border-slate-800">
                    <span className="text-slate-500 dark:text-slate-400 flex items-center gap-2"><FiPieChart /> Remaining Balance</span>
                    <span className="font-bold text-indigo-600 dark:text-indigo-400">₹{Math.max(0, (settings.totalFee || 0) - (settings.confirmationFee || 0)).toLocaleString()}</span>
                  </div>
                </div>
                ...
                <div className="pt-4 border-t border-slate-200 dark:border-slate-800">
                  <div className="flex justify-between items-center mb-3">
                    <div>
                      <h4 className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">Payment Plan</h4>
                    ...
                    <button 
                      onClick={() => { setSettingsModalTab("payment"); setShowRegistrationModal(true); }}
                      className="px-3 py-1.5 bg-indigo-50 dark:bg-indigo-600/20 hover:bg-indigo-100 dark:hover:bg-indigo-600/30 text-indigo-600 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-500/30 rounded-lg text-[10px] font-bold uppercase transition cursor-pointer"
                    >
                      Edit Payment Plan
                    </button>
                  </div>
                  <div className="space-y-3">
                    <div className="flex justify-between items-center text-xs p-2.5 rounded-lg bg-slate-50 dark:bg-[#0a101f] border border-slate-200 dark:border-slate-800">
                      <div className="flex items-center gap-2">
                        <span className="text-indigo-600 dark:text-indigo-400 font-bold">✓</span>
                        <div>
                          <p className="font-bold text-slate-900 dark:text-white">Confirmation Fee</p>
                          <p className="text-[9px] text-slate-500">At Registration</p>
                        </div>
                      </div>
                      <div className="text-right">
                        <div className="font-bold text-slate-900 dark:text-white">₹{settings.confirmationFee?.toLocaleString() || 0}</div>
                      </div>
                    </div>
                    {paymentPlanConfig && paymentPlanConfig.length > 0 ? (
                      paymentPlanConfig.map((inst, i) => (
                        <div key={i} className="flex justify-between items-center text-xs p-2.5 rounded-lg bg-slate-50 dark:bg-[#0a101f] border border-slate-200 dark:border-slate-800">
                          <div className="flex items-center gap-2">
                            <span className="text-indigo-600 dark:text-indigo-400 font-bold">○</span>
                            <div>
                              <p className="font-bold text-slate-900 dark:text-white">{inst.name || `Installment ${i+1}`}</p>
                          ...
                          <div className="text-right">
                            <div className="font-bold text-slate-900 dark:text-white">₹{inst.amount?.toLocaleString() || 0}</div>
                          </div>
                        </div>
                      ))
                    ) : (
                      <div className="p-3 bg-slate-50 dark:bg-[#0a101f] border border-slate-200 dark:border-slate-800 rounded-lg text-center text-[11px] text-slate-500">
              ...
              <div className="bg-white dark:bg-[#131c31] border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-xs dark:shadow-lg transition-colors">
                ...
                <div className="space-y-4">
                  {/* Operator */}
                  <div className="flex items-center justify-between p-3 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-[#0a101f]">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-slate-200 dark:bg-slate-800 flex items-center justify-center text-slate-600 dark:text-slate-400"><FiUsers /></div>
                      <div>
                        <div className="text-xs font-bold text-slate-900 dark:text-white">Operator</div>
                    ...
                    <button className="px-3 py-1.5 text-[10px] font-bold border border-slate-300 dark:border-slate-700 rounded-md text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-800 cursor-pointer">Manage</button>
                  </div>
                  ...
                  <div className="flex items-center justify-between p-3 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-[#0a101f]">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-slate-200 dark:bg-slate-800 flex items-center justify-center text-slate-600 dark:text-slate-400"><FiUsers /></div>
                      <div>
                        <div className="text-xs font-bold text-slate-900 dark:text-white">Students Access</div>
                        <div className={`flex items-center gap-1 text-[10px] font-semibold ${trip.studentAccess?.enabled ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-500'}`}>
                    ...
                    <button 
                      ...
                      className={`px-3 py-1.5 text-[10px] font-bold border rounded-md transition cursor-pointer ${trip.studentAccess?.enabled ? 'border-rose-300 dark:border-rose-500/30 text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-500/10' : 'border-indigo-300 dark:border-indigo-500/30 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-500/10'}`}
                    >
              ...
              <div className="bg-white dark:bg-[#131c31] border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-xs dark:shadow-lg transition-colors">
                ...
                <div className="flex flex-col gap-2">
                  <button onClick={() => navigate(`/itinerary/${trip._id}`, { state: { viewOnly: true, trip, dayIndex: 0, relation: "COORDINATOR" }})} className="flex items-center gap-3 w-full p-3 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800/50 transition text-left group cursor-pointer">
                    <div className="w-8 h-8 rounded-lg bg-indigo-50 dark:bg-indigo-900/40 text-indigo-600 dark:text-indigo-400 flex items-center justify-center group-hover:bg-indigo-600 group-hover:text-white transition"><FiEye /></div>
                    <div>
                      <div className="text-xs font-bold text-slate-800 dark:text-slate-200">View Finalized Itinerary</div>
                      <div className="text-[10px] text-slate-500">Read-only view</div>
                    </div>
                  </button>

                  <button onClick={() => navigate(`/builder?tripId=${trip._id}`)} className="flex items-center gap-3 w-full p-3 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800/50 transition text-left group cursor-pointer">
                    <div className="w-8 h-8 rounded-lg bg-indigo-50 dark:bg-indigo-900/40 text-indigo-600 dark:text-indigo-400 flex items-center justify-center group-hover:bg-indigo-600 group-hover:text-white transition"><FiEdit3 /></div>
                    <div>
                      <div className="text-xs font-bold text-slate-800 dark:text-slate-200">Update Itinerary</div>
        ...
        {showStudentModal && (() => {
          ...
          const configuredDocs = trip.documentsConfig?.filter(d => d.required) || [];
          const reqDocs = configuredDocs.length > 0 ? configuredDocs : defaultReqDocs;

          const unverifiedDocs = reqDocs.filter(req => {
            const reqName = (req.documentType || req.name || "").toLowerCase();
            const uploaded = showStudentModal.documents?.find(d => {
              const uType = (d.documentType || "").toLowerCase();
              return uType === reqName || 
                (reqName.includes("parent consent") && (uType.includes("parent consent") || uType.includes("undertaking"))) ||
                (reqName.includes("aadhaar") && uType.includes("aadhaar")) ||
                (reqName.includes("college id") && uType.includes("college id"));
            });
            return !uploaded || uploaded.status !== "VERIFIED";
          });

          const confirmationFeeAmount = settings.confirmationFee || 1000;
          const isConfirmationPaid = showStudentModal.confirmationPayment?.status === "PAID" || 
            showStudentModal.payments?.some(py => py.status === "PAID" && (py.installmentId === "CONFIRMATION" || py.amount === confirmationFeeAmount || py.name === "Confirmation Fee"));
          ...
          if (unverifiedDocs.length > 0) {
            missingRequirements.push(`Verification pending: ${unverifiedDocs.map(d => d.documentType || d.name).join(", ")}`);
          }
          ...
          return (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
              <div className="w-full max-w-3xl bg-white dark:bg-[#131c31] border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-2xl flex flex-col max-h-[92vh]">
                ...
                <div className="p-6 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-[#0a101f]">
                  <div>
                    <div className="flex items-center gap-3">
                      ...
                      <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wider ${
                        reviewStatus === 'APPROVED' ? 'bg-indigo-50 dark:bg-indigo-500/20 text-indigo-600 dark:text-indigo-400 border border-indigo-200 dark:border-indigo-500/30' :
                        reviewStatus === 'REJECTED' ? 'bg-rose-50 dark:bg-rose-500/20 text-rose-600 dark:text-rose-400 border border-rose-200 dark:border-rose-500/30' :
                  ...
                  <button 
                    ...
                    className="text-slate-400 hover:text-slate-700 dark:hover:text-white p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition cursor-pointer"
                  >
                ...
                {/* Modal Body */}
                <div className="p-6 overflow-y-auto space-y-8 text-slate-900 dark:text-white">

                  {/* 1. STUDENT DETAILS */}
                  <section>
                    <h4 className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-4 tracking-wider flex items-center gap-2">
                      <FiUsers className="text-indigo-600 dark:text-indigo-400" />
                      Student Details
                    </h4>
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4 bg-slate-50 dark:bg-[#0a101f] border border-slate-200 dark:border-slate-800/80 p-4 rounded-xl text-xs">
                      <div>
                        <p className="text-[10px] font-bold text-slate-500 uppercase">Full Name</p>
                        <p className="font-semibold text-slate-900 dark:text-white mt-1">{showStudentModal.studentInfo?.name || showStudentModal.userId?.name || '—'}</p>
                      </div>
                      <div>
                        <p className="text-[10px] font-bold text-slate-500 uppercase">Student ID / Roll No</p>
                        <p className="font-semibold text-slate-900 dark:text-white mt-1 font-mono">{showStudentModal.studentInfo?.studentId || showStudentModal.studentInfo?.rollNo || '—'}</p>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <p className="font-semibold text-slate-900 dark:text-white mt-1">{showStudentModal.studentInfo?.phone || showStudentModal.studentInfo?.studentPhone || '—'}</p>
                      </div>
                      <div>
                        <p className="text-[10px] font-bold text-slate-500 uppercase">Parent / Guardian Phone</p>
                        <p className="font-semibold text-slate-900 dark:text-white mt-1">{showStudentModal.studentInfo?.parentPhone || showStudentModal.studentInfo?.guardianPhone || '—'}</p>
                      </div>
                      <div>
                        <p className="text-[10px] font-bold text-slate-500 uppercase">Email Address</p>
                        <p className="font-semibold text-slate-900 dark:text-white mt-1 break-all">{showStudentModal.studentInfo?.email || showStudentModal.userId?.email || '—'}</p>
                      </div>
                      <div>
                        <p className="text-[10px] font-bold text-slate-500 uppercase">Emergency Contact</p>
                        <p className="font-semibold text-slate-900 dark:text-white mt-1">{showStudentModal.studentInfo?.emergencyContact || '—'}</p>
                      </div>
                      <div>
                        <p className="text-[10px] font-bold text-slate-500 uppercase">Dietary Restrictions</p>
                        <p className="font-semibold text-slate-900 dark:text-white mt-1">{showStudentModal.studentInfo?.foodAllergy || showStudentModal.studentInfo?.dietaryRestrictions || 'None'}</p>
                      </div>
                      <div>
                        <p className="text-[10px] font-bold text-slate-500 uppercase">Medical Information</p>
                        <p className="font-semibold text-slate-900 dark:text-white mt-1">{showStudentModal.studentInfo?.medicalInfo || 'None'}</p>
                      </div>
                      ...
                      {customEntries.length > 0 && customEntries.map(([key, val]) => (
                        <div key={key}>
                          ...
                          <p className="font-semibold text-slate-900 dark:text-white mt-1">{String(val)}</p>
                        </div>
                      ))}
                    </div>
                  </section>
                  ...
                  <section>
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <span className="text-[10px] bg-slate-100 dark:bg-slate-800 px-2.5 py-1 rounded text-slate-700 dark:text-slate-300 font-semibold">
                    ...
                    <div className="space-y-3">
                      {showStudentModal.documents?.length > 0 ? (
                        showStudentModal.documents.map((doc, i) => {
                          ...
                          return (
                            <div key={doc._id || i} className="flex flex-col md:flex-row md:items-center justify-between p-4 rounded-xl bg-slate-50 dark:bg-[#0a101f] border border-slate-200 dark:border-slate-800 gap-4">
                              <div className="flex items-center gap-3">
                                <div className={`p-2.5 rounded-lg shrink-0 ${
                                  isVerified ? 'bg-indigo-50 dark:bg-indigo-500/20 text-indigo-600 dark:text-indigo-400' : 
                                  isRejected ? 'bg-rose-50 dark:bg-rose-500/20 text-rose-600 dark:text-rose-400' : 
                                  'bg-amber-50 dark:bg-amber-500/20 text-amber-600 dark:text-amber-400'
                                }`}>
                                  <FiFileText className="text-lg" />
                                </div>
                                <div>
                                  <div className="text-sm font-bold text-slate-900 dark:text-white">{doc.documentType}</div>
                                  <div className="flex items-center gap-2.5 mt-1">
                                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded ${
                                      isVerified ? 'bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border border-indigo-200 dark:border-indigo-500/20' : 
                                      isRejected ? 'bg-rose-50 dark:bg-rose-500/10 text-rose-600 dark:text-rose-400 border border-rose-200 dark:border-rose-500/20' : 
                                      'bg-amber-50 dark:bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-200 dark:border-amber-500/20'
                                    }`}>
                                      {doc.status}
                                    </span>
                                    <button 
                                      ...
                                      className="text-xs font-semibold text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 flex items-center gap-1 transition cursor-pointer"
                                    >
                                      <FiEye className="text-xs" /> View Document
                                    </button>
                                  </div>
                                  {isRejected && doc.rejectionReason && (
                                    <p className="text-[11px] text-rose-600 dark:text-rose-400 mt-1.5 italic">
                              ...
                              <div className="flex gap-2 shrink-0">
                                {doc.status !== 'VERIFIED' && (
                                  <button 
                                    ...
                                    className="px-3 py-1.5 bg-indigo-50 dark:bg-indigo-600/20 hover:bg-indigo-100 dark:hover:bg-indigo-600/30 text-indigo-600 dark:text-indigo-400 border border-indigo-200 dark:border-indigo-500/30 rounded-lg text-xs font-bold transition flex items-center gap-1 cursor-pointer"
                                  >
                                ...
                                {doc.status !== 'REJECTED' && (
                                  <button 
                                    ...
                                    className="px-3 py-1.5 bg-rose-50 dark:bg-rose-600/20 hover:bg-rose-100 dark:hover:bg-rose-600/30 text-rose-600 dark:text-rose-400 border border-rose-200 dark:border-rose-500/30 rounded-lg text-xs font-bold transition flex items-center gap-1 cursor-pointer"
                                  >
                                    <FiX /> Reject
                                  </button>
                                )}
                        ...
                        <div className="p-4 rounded-xl bg-slate-50 dark:bg-[#0a101f] border border-slate-200 dark:border-slate-800 text-center text-xs text-slate-500">
                        ...
                        <div>
                          ...
                          <div className="mt-1">
                            <span className={`inline-flex items-center gap-1 text-[11px] font-bold px-2 py-0.5 rounded ${
                              isConfirmationPaid ? 'bg-indigo-50 dark:bg-indigo-500/20 text-indigo-600 dark:text-indigo-400' : 'bg-amber-50 dark:bg-amber-500/20 text-amber-600 dark:text-amber-400'
                            }`}>
                  ...
                  <section>
                    ...
                    <div className="bg-slate-50 dark:bg-[#0a101f] border border-slate-200 dark:border-slate-800 rounded-xl p-4 space-y-3">
                      ...
                      <textarea
                        ...
                        className="w-full bg-white dark:bg-[#131c31] border border-slate-300 dark:border-slate-700 text-xs text-slate-900 dark:text-white rounded-xl p-3 focus:outline-none focus:border-indigo-500 placeholder-slate-400 dark:placeholder-slate-500"
                      />
                  ...
                  <section className="p-5 rounded-xl bg-slate-50 dark:bg-[#0a101f] border border-slate-200 dark:border-slate-800 space-y-4">
                    <div className="flex items-center justify-between">
                      <div>
                        <h4 className="text-sm font-bold text-slate-900 dark:text-white">Coordinator Approval Decision</h4>
                        <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                    ...
                    <div className="bg-white dark:bg-[#131c31] p-3 rounded-lg text-xs space-y-1.5 border border-slate-200 dark:border-slate-800">
                      <div className="flex items-center gap-2">
                        {hasStudentDetails ? <FiCheckCircle className="text-indigo-600 dark:text-indigo-400 text-sm" /> : <FiAlertCircle className="text-amber-500 text-sm" />}
                        <span className={hasStudentDetails ? "text-slate-700 dark:text-slate-300" : "text-amber-600 dark:text-amber-400"}>Student Details Submitted</span>
                      </div>
                      <div className="flex items-center gap-2">
                        {unverifiedDocs.length === 0 ? <FiCheckCircle className="text-indigo-600 dark:text-indigo-400 text-sm" /> : <FiAlertCircle className="text-amber-500 text-sm" />}
                        <span className={unverifiedDocs.length === 0 ? "text-slate-700 dark:text-slate-300" : "text-amber-600 dark:text-amber-400"}>
                          All Required Documents Verified {unverifiedDocs.length > 0 && `(Missing: ${unverifiedDocs.map(d => d.documentType || d.name).join(", ")})`}
                        </span>
                      </div>
                      <div className="flex items-center gap-2">
                        {isConfirmationPaid ? <FiCheckCircle className="text-indigo-600 dark:text-indigo-400 text-sm" /> : <FiAlertCircle className="text-amber-500 text-sm" />}
                        <span className={isConfirmationPaid ? "text-slate-700 dark:text-slate-300" : "text-amber-600 dark:text-amber-400"}>Confirmation Fee Paid (Verified)</span>
                      </div>
                    </div>

                    {!canApprove && (
                      <div className="p-3 bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 rounded-lg text-xs text-amber-700 dark:text-amber-400 flex items-start gap-2">
                    ...
                    {reviewStatus === 'APPROVED' ? (
                      <div className="p-4 bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-200 dark:border-indigo-500/20 rounded-xl flex items-center gap-3 text-indigo-700 dark:text-indigo-400">
                      ...
                      <div className="flex flex-wrap items-center gap-3 pt-2">
                        <button 
                          ...
                          className={`px-5 py-2.5 rounded-xl font-bold text-xs transition flex items-center gap-2 shadow-md ${
                        ...
                        <button 
                          ...
                          disabled={actionLoading}
                          className="px-4 py-2.5 bg-rose-50 dark:bg-rose-600/20 hover:bg-rose-100 dark:hover:bg-rose-600/30 text-rose-600 dark:text-rose-400 border border-rose-200 dark:border-rose-500/30 rounded-xl font-bold text-xs transition flex items-center gap-2 cursor-pointer"
                        >
        ...
        {previewModal.isOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-4">
            <div className="w-full max-w-4xl bg-white dark:bg-[#131c31] border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
              {/* Header */}
              <div className="p-4 md:p-6 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-[#0a101f]">
                <div>
                  <h3 className="text-base font-bold text-slate-900 dark:text-white flex items-center gap-2">
                    <FiFileText className="text-indigo-600 dark:text-indigo-400" />
                    {previewModal.title}
                  </h3>
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Secure Document Preview</p>
                </div>
                <button
              ...
              {/* Body */}
              <div className="p-4 md:p-6 flex-1 overflow-auto flex items-center justify-center min-h-[300px] bg-slate-100 dark:bg-[#070b14]">
                {previewModal.loading && (
                  <div className="text-center py-12">
                    ...
                    <p className="text-xs text-slate-500 dark:text-slate-400 font-semibold">Decrypting and loading document preview...</p>
                  </div>
                )}

                {previewModal.error && (
                  <div className="text-center py-12 max-w-md">
                    <FiAlertCircle className="text-rose-500 dark:text-rose-400 text-3xl mx-auto mb-3" />
                    <p className="text-sm font-bold text-slate-900 dark:text-white mb-1">Failed to load preview</p>
                    <p className="text-xs text-rose-500 dark:text-rose-400">{previewModal.error}</p>
                  </div>
                )}
              ...
              {/* Footer */}
              <div className="p-4 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-[#0a101f] flex justify-end">
                <button
                  onClick={closePreviewModal}
                  className="px-4 py-2 bg-slate-200 hover:bg-slate-300 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-800 dark:text-white rounded-lg text-xs font-bold transition cursor-pointer"
                >
                  Close Preview
                </button>
              </div>
