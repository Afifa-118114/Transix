...
export default function StayPlanPage() {
  ...
  useEffect(() => {
    ...
    // IMPORTANT: Key on location string (not segment.id) so that changing a
    // segment's location from "New Destination" → "Munnar" always triggers
    // a fresh fetch rather than reusing a stale/empty result.
    staySegments.forEach((segment) => {
      const loc = (segment.location || "").trim();

      // Skip placeholder / invalid locations
      if (!loc || loc.toLowerCase() === "new destination") return;

      const segmentKey = loc.toLowerCase();

      // Skip if already loading
      if (loadingMap[segmentKey]) return;

      // Skip if hotel already selected for this segment
      if (segment.selectedHotel) return;

      // Skip if we already have a non-empty result for this location.
      // NOTE: We use Array.isArray check so that an empty [] does NOT block
      // a re-fetch — we allow one retry if the initial result was empty.
      const cached = segmentData[segmentKey];
  ...
  const handleSaveEdit = async (idx) => {
     const prevLocation = staySegments[idx].location;
     let validatedLocation = editForm.location.trim();
     let locationChanged = prevLocation !== validatedLocation || validatedLocation === "New Destination";
     ...
     if (locationChanged) {
        ...
        try {
          ...
          // Use "Tourist attractions" category — more reliable for smaller destinations
          // like Munnar, Alleppey, Coorg etc. than "city" which can miss them.
          const places = await getPlaces(validatedLocation, "Tourist attractions", token);

          if (!places || places.length === 0) {
            alert("Could not resolve this location. Please enter a valid geographic destination (e.g. Munnar, Goa, Shimla).");
            setIsSyncing(false);
            return;
          }

          // Try to extract the city/area name from the first result
          const firstAddr = places[0].formattedAddress || "";
          const firstParts = firstAddr.split(",").map(p => p.trim());

          // Heuristic: the canonical place name is what the user typed (if it appears in the address)
          // OR the first part of the formatted address (city/area level).
          const inputLower = validatedLocation.toLowerCase();
          const addrLower = firstAddr.toLowerCase();

          // Only reject if the canonical name is clearly a hotel/restaurant (not a city)
          const isBusiness = ["hotel", "resort", "boutique", "guest house", "homestay", "restaurant"]
            .some(w => inputLower.includes(w));

          if (isBusiness) {
            alert("Please enter a geographic destination/city name, not a hotel or restaurant name.");
            setIsSyncing(false);
            return;
          }

          // If the user's input appears anywhere in the formatted address, it's valid — keep it as-is.
          // Otherwise use the first segment of the address as the canonical location.
          if (!addrLower.includes(inputLower)) {
            // Fallback to shortest matching part of address that includes a keyword
            const matchingPart = firstParts.find(p => p.toLowerCase().includes(inputLower) || inputLower.includes(p.toLowerCase()));
            if (matchingPart) {
              validatedLocation = matchingPart;
            }
            // else keep the user's input — don't silently reject valid places
          }
        } catch (err) {
          console.error("Location validation failed:", err);
          // Don't block saving on validation error — allow the location through
        }
        setIsSyncing(false);
     }
     ...
     // Clear stale segmentData for the old location so a fresh fetch triggers
     if (locationChanged) {
       const oldKey = (prevLocation || "").toLowerCase();
       setSegmentData((prev) => {
         const updated = { ...prev };
        ...
        <div className="flex flex-col gap-8">
          {staySegments.length === 0 ? (
            ...
            staySegments.map((segment, index) => {
              ...
              // MUST match the key used in the fetch useEffect: location.toLowerCase()
              const dataKey = (segment.location || "").toLowerCase();
                  ...
                  {segment.selectedHotel ? (
                    <div className="relative overflow-hidden rounded-2xl border border-emerald-200 dark:border-emerald-900/50 bg-emerald-50/50 dark:bg-emerald-900/10 p-5 shadow-xs">
                      ...
                      <div className="mb-3 flex items-center gap-2">
                        <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-emerald-600 text-white text-[10px] font-black tracking-wider uppercase shadow-xs">
                          <span>⚡</span> Auto-Selected Stay
                        </span>
                        <span className="text-[11px] font-semibold text-emerald-700 dark:text-emerald-400">
                          Automated for 1-Click Checkout
                        </span>
                      </div>
                      <div className="flex flex-col sm:flex-row gap-4">
                        <div className="w-full sm:w-32 h-24 rounded-lg overflow-hidden shrink-0 bg-slate-200 dark:bg-slate-800">
                          <img
                            src={segment.selectedHotel.image || "https://images.unsplash.com/photo-1566073771259-6a8506099945?auto=format&fit=crop&w=800&q=80"}
