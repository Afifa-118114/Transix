/**
 * Bus Requirement Detector (Frontend)
 *
...
export const PERSONAL_CAR_OPTIONS = [
...
export const PERSONAL_MINIBUS_OPTIONS = [
...
export function cleanLocation(str) {
  if (!str) return "";
  return str
...
export function resolveItemRoute(item, fallbackFrom = "", fallbackTo = "") {
...
export function calculateDayDate(startDateStr, dayNumber) {
...
export function calculateCampusFleetRequirements({
  studentsCount = 200,
...
export function resolveLocalTransportArrangement(trip) {
  if (!trip) return null;
 ...
 */
export function detectBusRequirements(trip) {
  if (!trip) return [];
