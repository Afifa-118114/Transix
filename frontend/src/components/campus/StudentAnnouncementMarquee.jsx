...
export default function StudentAnnouncementMarquee({ announcements = [] }) {
  ...
  if (activeAnnouncements.length === 0) {
    return (
      <div className="flex items-center gap-3 bg-[#131c31] border border-slate-800 p-3 rounded-xl mb-6 text-slate-400 text-xs font-medium">
  ...
  return (
    <>
      ...
      <div className="relative flex items-center overflow-hidden bg-gradient-to-r from-indigo-950/70 via-[#131c31] to-indigo-950/70 border border-indigo-500/30 p-2 sm:p-2.5 rounded-xl mb-6 shadow-md group">
        {/* Left Fixed Badge / Label */}
        <div className="flex items-center gap-1.5 shrink-0 z-10 bg-indigo-600/90 text-white px-2.5 py-1 rounded-lg text-xs font-bold mr-3 shadow-xs">
          ...
          <div className="inline-flex items-center animate-marquee-scroll group-hover:[animation-play-state:paused]">
            {displayItems.map((ann, idx) => (
              <button
                ...
                className="inline-flex items-center gap-3 shrink-0 text-xs font-medium text-indigo-100 hover:text-white hover:underline underline-offset-4 cursor-pointer transition-colors px-2 py-0.5 focus:outline-none"
              >
                <span className="truncate max-w-xs sm:max-w-md md:max-w-lg">
                  {ann.message}
                </span>
                <span className="text-indigo-400 font-black text-sm px-2 select-none" aria-hidden="true">
                  •
                </span>
              </button>
            ))}
      ...
      {selectedAnnouncement && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 animate-in fade-in duration-150">
          <div className="w-full max-w-md bg-[#131c31] border border-slate-800 rounded-2xl p-6 shadow-2xl text-white">
            ...
            <div className="bg-[#0a101f] border border-slate-800/80 rounded-xl p-4 mb-4">
