    ...
    <div className="flex h-full flex-col rounded-2xl border border-slate-200/80 bg-white p-4 shadow-xs">
      ...
      <div className="mb-3">
        <div className="flex items-center justify-between">
          <h2 className="text-base font-bold text-slate-900">Available Options</h2>
      ...
      <div className="relative mb-2.5">
        ...
        <input
          ...
          className="w-full rounded-xl border border-slate-200 bg-slate-50/80 py-1.5 pl-8 pr-7 text-xs text-slate-800 outline-none transition focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-100"
        />
        {searchTerm && (
          <button
        ...
        {CATEGORIES.map((cat) => {
          ...
          return (
            ...
            >
              <span>{cat.icon}</span>
              <span>{cat.label}</span>
              <span className={`text-[9px] px-1 py-0.2 rounded-full ${isSelected ? "bg-indigo-500 text-white" : "bg-slate-200 text-slate-600"}`}>
                {count}
              </span>
            </button>
          );
      ...
      <div className="flex-1 space-y-2.5 overflow-y-auto pr-1">
        {isLoadingInventory ? (
          <div className="space-y-3">
            {[1, 2, 3].map((i) => (
              <div key={i} className="h-32 animate-pulse rounded-xl bg-slate-100 border border-slate-200" />
            ))}
          </div>
          ...
          <div className="flex h-36 flex-col items-center justify-center rounded-xl border border-dashed border-slate-200 p-4 text-center">
            <p className="text-xs font-semibold text-slate-600">No matching options</p>
            <p className="mt-0.5 text-[10px] text-slate-400">Try adjusting category or search term</p>
          </div>
        ) : (
          filteredItems.map((item) => (
            <div
              ...
              className="group relative cursor-grab overflow-hidden rounded-xl border border-slate-200/80 bg-white p-2.5 shadow-xs transition-all duration-200 hover:border-indigo-300 hover:shadow-sm active:cursor-grabbing"
            >
              {/* Thumbnail & Badges */}
              ...
              <div className="mt-2">
                ...
                <div className="mt-0.5 flex items-center justify-between text-[10px] text-slate-500">
                  <span className="flex items-center gap-0.5 truncate">
                    <FiMapPin className="text-indigo-500 text-[9px] shrink-0" />
                    <span className="truncate">{item.location || trip.destination}</span>
                  </span>
                  <span className="flex items-center gap-0.5 shrink-0 text-slate-400 ml-1">
                ...
                <div className="mt-2 flex gap-1.5">
                  <button
                    ...
                    className="flex flex-1 items-center justify-center gap-1 rounded-lg bg-indigo-50 py-1 text-[11px] font-bold text-indigo-700 transition hover:bg-indigo-600 hover:text-white"
                  >
                  ...
                  <div className="relative">
                    <button
                      ...
                      title="Select Day"
                      className="flex h-full items-center rounded-lg border border-slate-200 px-1.5 text-slate-500 hover:bg-slate-100"
                    >
                      <FiChevronDown className="text-xs" />
                    </button>

                    {activeMenuId === item.id && (
                      <div className="absolute bottom-full right-0 z-30 mb-1 w-32 rounded-xl border border-slate-200 bg-white p-1 shadow-lg">
                        <p className="px-2 py-0.5 text-[9px] font-bold uppercase text-slate-400">
                          Select Day:
                        </p>
                        {trip.itinerary.map((d, dIdx) => (
                          <button
                            ...
                            className="flex w-full items-center justify-between rounded-md px-2 py-1 text-left text-[11px] font-semibold text-slate-700 hover:bg-indigo-50 hover:text-indigo-600"
                          >
